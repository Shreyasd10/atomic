...
describe("buildGraphOverlayAdapter — animation tick visibility gating", () => {
	test("requestRender from the view fires tui.requestRender while visible", async () => {
		...
		try {
			...
			assert.ok(renderCalls >= 2, `expected tui.requestRender to fire on the animation tick (got ${renderCalls})`);
		} finally {
			component!.dispose?.();
		}
	}, 15_000);
	...
	test("tick stops after the component is disposed", async () => {
		...
		await new Promise((r) => setTimeout(r, 250));
		assert.equal(renderCalls, afterDispose, "no further ticks should fire after dispose");
	});
});
