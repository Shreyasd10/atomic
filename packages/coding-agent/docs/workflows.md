...
At 80 columns and wider, each `BACKGROUND` card keeps the full run identity and preserves its mode, progress, live-tool details, elapsed/status metadata, and any pending-stage projection that fits the existing metadata-row budget. A waiting card with exactly one displayable pending HIL request in its visible run tree adds one quoted, terminal-control-stripped, cell-bounded question row and an action row with the exact `/workflow connect <full-run-id>` command. The command targets the visible top-level run even when the concrete prompt belongs to a nested child, and the full UUID wraps rather than being ellipsized. `F2 answer` appears only when F2 currently targets that visible run; the programmatic path remains `workflow answer` with exact run, stage, and prompt identity. Promptless waits, multi-question requests, and multiple pending occurrences keep the ordinary status-only card. Pending-stage targets remain exact when shown, or fall back to a `stage`-labeled canonical ID; `… N more` reports omitted pending stages. If no bounded pending-stage form fits, the pending label is omitted rather than displacing live-tool or elapsed/status metadata. Tool nodes are read-only durable graph nodes, not attachable stage chats. Below 80 columns, the panel keeps its aggregate collapsed form and omits run IDs, stage identities, targets, prompt text, and tool names.
