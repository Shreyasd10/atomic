...
- Added fullscreen transcript search. `Ctrl+Shift+F` opens a find box over the rendered transcript; typing filters as you go, `enter`/`ctrl+g` select the next match, `shift+enter`/`ctrl+shift+g` the previous one, and `escape` closes the box. While the find box has focus it keeps `escape`, `enter`, and `ctrl+g`, so those keys navigate the search instead of interrupting the turn, submitting the editor, or opening the external editor; `pageUp`, `pageDown`, `home`, `end`, and the wheel still scroll the transcript underneath. Matches highlight with two new optional theme colors, `searchMatchBg` and `searchMatchText`, falling back to `selectedBg` and `text` so every existing theme keeps validating; the current match reverses that pair in bold. A focused overlay that owns a transcript of its own — an attached workflow stage chat — is offered the key first and searches that transcript instead of the main one behind it. Two new unbound viewport actions, `tui.altScreen.lineUp` and `tui.altScreen.lineDown`, scroll the transcript by a single line ([#2385](https://github.com/bastani-inc/atomic/issues/2385)).
...
- The exported `Theme` constructor's color maps now accept the optional `searchMatchText` (foreground) and `searchMatchBg` (background) tokens alongside `scrollbarThumb`, with the same fallbacks encoded in the typebox and JSON theme schemas, so every user-authored theme keeps validating and rendering.
...
- Fullscreen viewport keys keep their Atomic routing on pi-tui 0.84.2. A focused overlay is still offered viewport keys first and the transcript still receives what it declines — including behind the `ask_user_question` dialog — while pi-tui's own find box is exempted so the transcript keeps scrolling while a search is open; a host action bound to a printable key is dispatched before the find box can type it into the query; and every settled overlay answer other than `true` — an async handler resolving `undefined`, a rejection, or an overlay with no input handler at all — is treated as a decline and replayed to the transcript.
